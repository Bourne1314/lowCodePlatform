package com.csicit.ace.webservice.commonUtils;
...
public class SqlUtils {
    ...
    public void validateTreeTable(String tableName, String parentId, Integer sortIndex, String sortPath) {
        // 校验排序号
        int count = dbHelperMapper.getCountWithParams("select count(*) from {0} where sort_index={2} and " +
    ...
    public Integer updateSonSortPath(String tableName, String parentNewPath, Integer parentOldPathLength, String
            parentOldPath) {
        return dbHelperMapper.updateBySqlWithParams("update {0} set sort_path=concat(''{1}'',SUBSTR(sort_path," +
                "{2} + 1)) \n" +
    ...
    public String verifyParentId(String tableName, String parentId, String id, int sortIndex) {
         ...
         * 判断同一个父节点下是否存在同样的排序号
         */
        int sortIndexNum = dbHelperMapper.getCountWithParams("select count(*) from {0} where sort_index=''{3}'' and " +
                        "id!=''{1}'' and" +
                        " " +
                        "parent_id=''{2}'' "
         ...
         * 判断parent_id是否改变
         */
        int num = dbHelperMapper.getCountWithParams("select count(*) from {0} where sort_index=''{3}'' and id=''{1}''" +
                        " and" +
                        " parent_id=''{2}''"
                , tableName, id, parentId, sortIndex + "");
        if (num == 1) {
        ...
        String parentPath = dbHelperMapper.getStringWithParams("select sort_path from {0} where id=''{1}'' ",
                tableName, parentId);
        String sonPath = dbHelperMapper.getStringWithParams("select sort_path from {0} where id=''{1}'' ",
                tableName, id);
    ...
    public Integer getMaxSort(Map<String, Object> params) {
        ...
        if (Objects.equals("org_organization", tableName)) {
            maxIndex = dbHelperMapper.getMaxSortNoWithParams("select max(sort_index) from {0} where " +
                            "group_id=''{1}''" +
        ...
        if (Objects.equals("sys_config", tableName)) {
            if (Objects.equals(1, scope)) {
                // 租户
                maxIndex = dbHelperMapper.getMaxSortNoWithParams("select max(sort_index) from {0} where scope=1",
                        tableName
                );
            } else if (Objects.equals(2, scope)) {
                maxIndex = dbHelperMapper.getMaxSortNoWithParams("select max(sort_index) from {0} where scope=2 and " +
                                "group_id=''{1}''",
                        tableName, groupId
                );
            } else if (Objects.equals(3, scope)) {
                maxIndex = dbHelperMapper.getMaxSortNoWithParams("select max(sort_index) from {0} where scope=3 and " +
        ...
        if (Objects.equals("sys_dict_value", tableName)) {
            if (Objects.equals(1, scope)) {
                // 租户
                maxIndex = dbHelperMapper.getMaxSortNoWithParams("select max(sort_index) from {0} where scope=1 and " +
        ...
        if (Objects.equals("report_type", tableName)) {
            maxIndex = dbHelperMapper.getMaxSortNoWithParams("select max(sort) from {0} where " +
                            "app_id=''{1}'' and parent_id=''{2}''",
                    tableName, appId, parentId
            );
        }
        ...
        if (Objects.equals("wfd_flow_category", tableName)) {
            maxIndex = dbHelperMapper.getMaxSortNoWithParams("select max(sort_no) from {0} where " +
                            "app_id=''{1}''",
                    tableName, appId
            );
        }
    ...
    public Integer getDevMaxSort(Map<String, Object> params) {
        ...
        // 开发平台-用户表
        if (Objects.equals("sys_user", tableName)) {
            maxIndex = dbHelperMapper.getMaxSortNoWithParams("select max(sort_index) from {0}",
                    tableName
            );
        }
        if (maxIndex == null) {
