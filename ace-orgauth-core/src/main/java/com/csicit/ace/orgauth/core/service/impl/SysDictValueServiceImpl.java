...
public class SysDictValueServiceImpl extends BaseServiceImpl<SysDictValueMapper, SysDictValueDO> implements
    ...
    @Override
    public List<SysDictValueDO> list(Map<String, Object> params) {
        ...
        if (StringUtils.isNotBlank(type)) {
            ...
            if (StringUtils.isNotBlank(appId)) {
                list = getDictValueByAppId(type, appId);
            } else if (StringUtils.isNotBlank(groupId)) {
                list = getDictValueByGroupId(type, groupId);
            } else {
                list = getDictValueByGroupId(type, securityUtils.getCurrentGroupId());
            }
    ...
    public List<SysDictValueDO> getDictValueByAppId(String type, String appId) {
        if (StringUtils.isNotBlank(type) && StringUtils.isNotBlank(appId)) {
            List<SysDictValueDO> list = list(new QueryWrapper<SysDictValueDO>().eq("scope", 3).eq("type", type).eq
                    ("app_id",
                            appId).orderByAsc("sort_path"));
            return !org.apache.commons.collections.CollectionUtils.isEmpty(list) ? list : getDictValueByGroupId(type, securityUtils
                    .getCurrentGroupId());
        }
        if (StringUtils.isNotBlank(type)) {
    ...
    public boolean saveDictValue(SysDictValueDO sysDictValueDO) {
        ...
        if (save(sysDictValueDO)) {
            return sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.success, "新增"), "新增字典数据项", "新增字典数据项："+sysDictValueDO.getDictName(), securityUtils
                            .getCurrentGroupId(),
...
//        }
        ...
        if (updateById(sysDictValueDO)) {
            return sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.warning, "更新"), "修改字典数据项","修改字典数据项："+ sysDictValueDO.getDictName(), securityUtils
                            .getCurrentGroupId(),
                    sysDictValueDO.getAppId());
        }
        return false;
    ...
    public boolean deleteByIds(Collection<? extends Serializable> ids) {
        ...
        return
                sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.danger, "删除"),
                        "删除字典数据", "删除字典数据："+stringBuffer.substring(0, stringBuffer.length() - 1), securityUtils
