...
import com.csicit.ace.orgauth.core.service.BdJobService;
...
public class BdJobServiceImpl extends BaseServiceImpl<BdJobMappper, BdJobDO> implements BdJobService {
    ...
    public boolean saveJob(BdJobDO jobDO) {
        ...
        if (save(jobDO)) {
            if (sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.success, "新增"), "保存职务称谓", "保存职务称谓："+jobDO.getName(), jobDO.getGroupId(), null)) {
                return true;
            } else {
    ...
    public boolean updateJob(BdJobDO jobDO) {
        ...
        if (updateById(jobDO)) {
            if (sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.warning, "更新"), "修改职务称谓", "修改职务称谓："+jobDO.getName(), jobDO.getGroupId(), null)) {
                return true;
            } else {
                throw new RException(InternationUtils.getInternationalMsg("UNKNOWN_EXCEPTION"));
            }
        ...
        if (ids.length > 0) {
            ...
            if (removeByIds(Arrays.asList(ids))) {
                if (sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.danger, "删除"), "删除职务称谓", "删除职务称谓："+list.stream().map
                        (BdJobDO::getName).collect(Collectors.toList()), list.get(0).getGroupId(), null)) {
                    return true;
                } else {
                    throw new RException(InternationUtils.getInternationalMsg("EMPTY_ARG"));
                }
