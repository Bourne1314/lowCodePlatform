...
import com.csicit.ace.orgauth.core.service.SysPasswordPolicyService;
 ...
 * @date 2019/4/3 17:45
 */
@Service("sysPasswordPolicyServiceO")
...
//        SysPasswordPolicyDO pswPolicy = (SysPasswordPolicyDO) cacheUtil.hget(appName, "passwordPolicy");
//        if (pswPolicy == null) {
    ...
    public R update(SysPasswordPolicyDO instance) {
        ...
        if (updateById(instance)) {
            if (!sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.warning, "更新"), "更新密码策略", "更新密码策略："+instance.getRemark())) {
    ...
    public R delete(String[] ids) {
        ...
        if (removeByIds(Arrays.asList(ids))) {
            if (!sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.danger, "删除"), "删除密码策略", "删除密码策略："+list.parallelStream().map
