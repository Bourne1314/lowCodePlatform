...
public class BdPostServiceImpl extends BaseServiceImpl<BdPostMapper, BdPostDO> implements BdPostService {
    ...
    public boolean savePost(BdPostDO postDO) {
        ...
        if (save(postDO)) {
                ...
                if (org.apache.commons.collections.CollectionUtils.isNotEmpty(depIds)) {
                    ...
                    depIds.forEach(depId -> {
                        if (StringUtils.isNotBlank(depId)) {
                            ...
                            // 岗位名不能相同
                            if (count(new QueryWrapper<BdPostDO>().eq("DEPARTMENT_ID", depId).eq("name", postDO
                                    .getName())) > 0) {
                                throw new RException(InternationUtils.getInternationalMsg("SAME_MODEL",
                                        new String[]{InternationUtils.getInternationalMsg("NAME"), postDO.getName()}
                                ));
                            }
                            if (!save(newPost)) {
            ...
            if (sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.success, "新增"), "保存岗位","保存岗位："+ postDO.getName(), postDO
                            .getGroupId(),
                    null)) {
                return true;
            } else {
    ...
    public boolean updatePost(BdPostDO postDO) {
        ...
        if (updateById(postDO)) {
            if (sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.warning, "更新"), "修改岗位", "修改岗位："+ postDO.getName(), postDO
                    .getGroupId(), null)) {
                return true;
            } else {
                throw new RException(InternationUtils.getInternationalMsg("UNKNOWN_EXCEPTION"));
    ...
    public boolean deletePost(String[] ids) {
        if (ids.length > 0) {
            ...
            if (removeByIds(Arrays.asList(ids))) {
                if (sysAuditLogService.saveLog(new AuditLogTypeDO(AuditLogType.danger, "删除"), "删除岗位", "删除岗位："+list.stream().map
                        (BdPostDO::getName)
