package com.csicit.ace.dbplus.service.impl;
...
public class BaseServiceImpl<M extends BaseMapper<T>, T> extends ServiceImpl<M, T> implements IBaseService<T> {
     ...
     * in查询最大列数
      */
    private static final int MAX_IN_COUNT = 1000;
    /**
     * 安全工具类对象
     */
    @Autowired
    ...
    public Collection<T> listByIds(Collection<? extends Serializable> idList) {
        if (idList != null && idList.size() > 0) {
            Object[] allIds = idList.toArray();
            int p = idList.size() / MAX_IN_COUNT;
            Collection<T> collection = new ArrayList<>();
            List<Serializable> ids = new ArrayList<>();
            for(int i = 0; i < p; i++)
            {
                ids.clear();
                for (int j = 0; j < MAX_IN_COUNT; j++)
                {
                    ids.add((Serializable) allIds[i * MAX_IN_COUNT + j]);
                }
                listByIds(collection, ids);
            }
            ids.clear();
            for(int i = p * MAX_IN_COUNT; i < allIds.length; i++)
            {
                ids.add((Serializable) allIds[i]);
                listByIds(collection, ids);
            }
            return collection;
        } else {
    ...
    private void listByIds(Collection<T> collection, List<Serializable> ids) {
        Collection<T> col = super.listByIds(ids);
        if(col.size() > 0)
        {
