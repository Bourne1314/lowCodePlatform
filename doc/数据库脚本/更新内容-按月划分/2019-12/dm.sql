...
CONSTRAINT "FK_DISPLAY_USER_ID" FOREIGN KEY("USER_ID") REFERENCES "SYS_USER"("ID") ON DELETE CASCADE ) STORAGE(ON "JARIACEPLAT", CLUSTERBTR) ;
...
"APP_ID" VARCHAR2(50),
NOT CLUSTER PRIMARY KEY("ID"),
CONSTRAINT "FK_WAIT_GRANT_USER_ID" FOREIGN KEY("USER_ID") REFERENCES "JARIACEPLAT"."SYS_USER"("ID") ON DELETE CASCADE ,
CONSTRAINT "FK_WAIT_GRANT_APP_ID" FOREIGN KEY("APP_ID") REFERENCES "JARIACEPLAT"."SYS_GROUP_APP"("ID") ON DELETE CASCADE ) STORAGE(ON "JARIACEPLAT", CLUSTERBTR) ;
COMMENT ON TABLE "JARIACEPLAT"."SYS_WAIT_GRANT_USER" IS '角色用户授予—待激活的用户表';
COMMENT ON COLUMN "JARIACEPLAT"."SYS_WAIT_GRANT_USER"."APP_ID" IS '应用ID';
COMMENT ON COLUMN "JARIACEPLAT"."SYS_WAIT_GRANT_USER"."ID" IS '主键';
COMMENT ON COLUMN "JARIACEPLAT"."SYS_WAIT_GRANT_USER"."USER_ID" IS '用户ID';
