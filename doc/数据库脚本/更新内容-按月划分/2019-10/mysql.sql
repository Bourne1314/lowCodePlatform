/**
  2019/10/08
 */
ALTER TABLE `JARIACEPLAT`.`SYS_AUTH_USER` ADD (`APP_ID`  VARCHAR(50) COMMENT '应用ID');
ALTER TABLE `JARIACEPLAT`.`SYS_AUTH_USER_V` ADD (`APP_ID`  VARCHAR(50) COMMENT '应用ID');
 ...
 */
alter table bd_person_id_type DROP foreign key FK_BD_PERSON_ID_TYPE_ORG_ID;
alter table bd_person_id_type DROP foreign key UK_BD_PERSON_ID_TYPE_NAME;
alter table bd_person_id_type DROP organization_id;
/**
 2019-10-14
 */
alter table ORG_ORGANIZATION_V_TYPE add column(`DATA_VERSION` INT(1) DEFAULT 0 COMMENT '数据版本');


/**
 2019-10-15
 */
alter table FILE_CONFIGURATION add column(`IS_ENABLE_EVT_FILE_DELETING` INT(1) COMMENT '是否启用事件文件删除前');
alter table FILE_CONFIGURATION add column(`IS_ENABLE_EVT_FILE_DOWNLOADING` INT(1) COMMENT '是否启用事件文件下载前');
alter table FILE_CONFIGURATION add column(`IS_ENABLE_EVT_FILE_UPLOADING` INT(1) COMMENT '是否启用事件文件上传前');
alter table FILE_CONFIGURATION add column(`IS_ENABLE_EVT_FILE_UPLOADED` INT(1) COMMENT '是否启用事件文件上传后（不影响文件上传到服务器）');
/**
...
alter table FILE_CONFIGURATION add column(`IS_ENABLE_EVT_FILE_DOWNLOADED` INT(1) COMMENT '是否启用文件下载后事件');
...
ALTER TABLE `JARIACEPLAT`.`SYS_USER_ROLE` ADD (`APP_ID`  VARCHAR(50) COMMENT '应用ID');

/**
 2019/10/22
 */
ALTER TABLE `JARIACEPLAT`.`SYS_USER_ROLE` ADD (`IS_ACTIVATED`  INT(1)  DEFAULT 0 COMMENT '是否被激活，0没有，1是');

ALTER TABLE `JARIACEPLAT`.`SYS_USER_ROLE_LV` ADD (`APP_ID`  VARCHAR(50) COMMENT '应用ID');
...
ALTER TABLE `JARIACEPLAT`.`FILE_CONFIGURATION` ADD (`DOWNLOAD_AUTH_CODE`  VARCHAR(50) COMMENT '下载权限标识');
ALTER TABLE `JARIACEPLAT`.`FILE_CONFIGURATION` ADD (`UPLOAD_AUTH_CODE`  VARCHAR(50) COMMENT '上传权限标识');
ALTER TABLE `JARIACEPLAT`.`FILE_CONFIGURATION` ADD (`DELETE_AUTH_CODE`  VARCHAR(50) COMMENT '删除权限标识');

ALTER TABLE `JARIACEPLAT`.`FILE_CONFIGURATION` MODIFY COLUMN `UPLOAD_OPERATION_KEY` VARCHAR(50) COMMENT '上传操作标识';
ALTER TABLE `JARIACEPLAT`.`FILE_CONFIGURATION` MODIFY COLUMN `DELETE_OPERATION_KEY` VARCHAR(50) COMMENT '删除操作标识';
ALTER TABLE `JARIACEPLAT`.`FILE_CONFIGURATION` MODIFY COLUMN `DOWNLOAD_OPERATION_KEY` VARCHAR(50) COMMENT '下载操作标识';

/**
2019/10/24
 */
alter table bd_person_id_type DROP foreign key  UK_ORG_ORGANIZATION_PARENT_ID_SORTINDEX;
/**
2019/10/27
 */
